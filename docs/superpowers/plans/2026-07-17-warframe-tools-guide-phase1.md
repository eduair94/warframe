...
- **Verified gate:** `verified = a real-browser screenshot exists`. A captured screenshot is stronger proof of "working" than an HTTP code — many live sites (Cloudflare-fronted: wiki.warframe.com, overframe, rivenradar, fandom) refuse datacenter server-fetches with 403/503 yet render fine in a real browser. `httpStatus` is stored as an informational reachability signal, not the gate. A tool with no clean screenshot stays `verified:false` (card shows an "unverified" state, not a broken live link) or is dropped if it won't load at all.
