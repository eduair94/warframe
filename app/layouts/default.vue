...
export default {
  ...
  computed: {
    ...
    drawerSections() {
      ...
      return order
        .map((g) => ({
          title: g,
          items: this.navLinks.filter((l) => (l.group || null) === g),
        }))
  ...
  watch: {
    // Keep the current page's tab visible when navigating between pages
    $route() {
      this.$nextTick(() => {
        this.centerActiveNav()
        this.updateNavFades()
      })
    },
  },
  methods: {
    ...
    async startTour() {
      ...
      const step = (to, title, description) => ({
    ...
    setupNav() {
      this.$nextTick(() => {
        const scroller = this.$el && this.$el.querySelector('.app-nav__scroll')
        if (!scroller) return
        scroller.addEventListener('scroll', this.updateNavFades, { passive: true })
        window.addEventListener('resize', this.updateNavFades)
        this.centerActiveNav()
        this.updateNavFades()
      })
    },
    centerActiveNav() {
      const scroller = this.$el && this.$el.querySelector('.app-nav__scroll')
      const active =
        scroller && scroller.querySelector('.app-nav__item.nuxt-link-exact-active')
    ...
    updateNavFades() {
      const scroller = this.$el && this.$el.querySelector('.app-nav__scroll')
      const nav = this.$el && this.$el.querySelector('.app-nav')
      if (!scroller || !nav) return
      const max = scroller.scrollWidth - scroller.clientWidth
      nav.classList.toggle('has-more-right', scroller.scrollLeft < max - 2)
      nav.classList.toggle('has-more-left', scroller.scrollLeft > 2)
    },
  },
}
</script>
