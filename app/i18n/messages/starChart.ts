...
export default {
  en: {
    starChart: {
      ...
      disclaimer: "Expected p/drop = Σ (drop chance × realizable value) across a mission's reward table. Realizable value uses each drop's 48h average sell price, weighted by its 48h trade volume (liquidity) — so overpriced drops nobody actually buys don't inflate a mission's worth. Drop chances come from community drop data; prices and volume are from Warframe Market. Untradeable rewards (Forma, resources, credits) count as zero.",
    },
  },
  es: {
    starChart: {
  ...
  fr: {
    starChart: {
      ...
      disclaimer: "p/drop attendu = Σ (chance de drop × valeur réalisable) sur toute la table de récompenses d'une mission. La valeur réalisable utilise le prix de vente moyen sur 48 h de chaque drop, pondéré par son volume d'échange sur 48 h (liquidité) — ainsi les drops surévalués que personne n'achète vraiment ne gonflent pas la valeur d'une mission. Les chances de drop proviennent des données de la communauté ; les prix et le volume viennent du Warframe Market. Les récompenses non échangeables (Forma, ressources, crédits) comptent pour zéro.",
      forma: {
        toggle: "Forma relics",
        note: "Highlighting {n} worlds that drop a Forma relic. Dimmed worlds have none.",
        listLink: "List every Forma relic",
  ...
  ko: {
    starChart: {
      ...
      disclaimer: "예상 p/drop = Σ (드랍 확률 × 실현 가능 가치), 임무의 전체 보상 테이블 기준. 실현 가능 가치는 각 드랍의 48시간 평균 판매가를 48시간 거래량(유동성)으로 가중한 값입니다 — 그래서 아무도 실제로 사지 않는 고평가 드랍이 임무 가치를 부풀리지 않습니다. 드랍 확률은 커뮤니티 드랍 데이터에서, 가격과 거래량은 Warframe Market에서 가져옵니다. 거래 불가 보상(Forma, 자원, 크레딧)은 0으로 계산합니다.",
      forma: {
        toggle: "Forma relics",
        note: "Highlighting {n} worlds that drop a Forma relic. Dimmed worlds have none.",
        listLink: "List every Forma relic",
  ...
  ja: {
    starChart: {
      ...
      disclaimer: "期待 p/drop = Σ (ドロップ確率 × 実現可能価値)、ミッションの報酬テーブル全体で算出。実現可能価値は各ドロップの48時間平均販売価格を、48時間の取引量(流動性)で加重した値です — そのため、誰も実際には買わない過大評価のドロップがミッションの価値を水増しすることはありません。ドロップ確率はコミュニティのドロップデータから、価格と取引量はWarframe Marketから取得しています。取引不可の報酬(Forma、リソース、クレジット)はゼロとして計算します。",
      forma: {
        toggle: "Forma relics",
        note: "Highlighting {n} worlds that drop a Forma relic. Dimmed worlds have none.",
        listLink: "List every Forma relic",
  ...
  'zh-hans': {
    starChart: {
      ...
      disclaimer: "预期 p/drop = Σ (掉落几率 × 可变现价值)，覆盖任务的整个奖励表。可变现价值采用每个掉落的 48 小时平均售价，并按其 48 小时交易量(流动性)加权 — 这样一来，没人真正购买的高价掉落不会虚增任务价值。掉落几率来自社区掉落数据；价格和交易量来自 Warframe Market。不可交易的奖励(Forma、资源、现金)按零计算。",
      forma: {
        toggle: "Forma relics",
        note: "Highlighting {n} worlds that drop a Forma relic. Dimmed worlds have none.",
        listLink: "List every Forma relic",
  ...
  'zh-hant': {
    starChart: {
      ...
      disclaimer: "預期 p/drop = Σ (掉落機率 × 可變現價值)，涵蓋任務的整個獎勵表。可變現價值採用每個掉落的 48 小時平均售價，並按其 48 小時交易量(流動性)加權 — 這樣一來，沒人真正購買的高價掉落不會虛增任務價值。掉落機率來自社群掉落資料；價格和交易量來自 Warframe Market。不可交易的獎勵(Forma、資源、現金)按零計算。",
      forma: {
        toggle: "Forma relics",
        note: "Highlighting {n} worlds that drop a Forma relic. Dimmed worlds have none.",
        listLink: "List every Forma relic",
  ...
  it: {
    starChart: {
      ...
      disclaimer: "p/drop atteso = Σ (probabilità di drop × valore realizzabile) sull'intera tabella delle ricompense di una missione. Il valore realizzabile usa il prezzo medio di vendita a 48h di ciascun drop, ponderato per il suo volume di scambi a 48h (liquidità) — così i drop sopravvalutati che nessuno compra davvero non gonfiano il valore di una missione. Le probabilità di drop provengono dai dati della community; i prezzi e il volume sono dal Warframe Market. Le ricompense non scambiabili (Forma, risorse, crediti) contano come zero.",
      forma: {
        toggle: "Forma relics",
        note: "Highlighting {n} worlds that drop a Forma relic. Dimmed worlds have none.",
        listLink: "List every Forma relic",
