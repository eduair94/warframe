...
if command -v pm2 >/dev/null 2>&1; then
  ...
  if pm2 save >/dev/null 2>&1; then
    ok "process list saved ($(pm2 jlist 2>/dev/null | grep -o '"pm_id"' | wc -l) apps) -> /root/.pm2/dump.pm2"
  else
...
if systemctl list-unit-files 2>/dev/null | grep -q '^cloudflared\.service'; then
  ...
  # so it must restart itself rather than wait for a human.
  if systemctl show cloudflared -p Restart --value 2>/dev/null | grep -qE 'always|on-failure'; then
    ok "auto-restart: $(systemctl show cloudflared -p Restart --value)"
  else
...
UNIT
    ...
    ok "auto-restart override installed (Restart=always)"
  fi
...
if crontab -l 2>/dev/null | grep -qF 'warframe-watchdog.sh'; then
